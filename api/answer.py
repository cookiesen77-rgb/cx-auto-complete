...
class Tiku:
    ...
    def init_tiku(self):
        ...
        if not self.DISABLE:
            ...
            self.true_list = self._conf.get('true_list', '正确,对,√,是').split(',')
            self.false_list = self._conf.get('false_list', '错误,错,×,否,不对,不正确').split(',')
            # 调用自定义题库初始化
            self._init_tiku()
...
class AI(Tiku):
    ...
    def _init_tiku(self):
        # 处理 endpoint：去掉 /chat/completions（OpenAI SDK 会自动添加）
        endpoint = self._conf['endpoint']
        self.endpoint = endpoint.replace('/chat/completions', '').rstrip('/')
